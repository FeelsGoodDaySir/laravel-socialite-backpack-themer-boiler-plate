		...
		<div class="w-full h-100">
			...
			<form class="mt-6" action="{{ route('login') }}" method="POST">
				...
				<div>
					...
					<input type="email" name="email" id="email" placeholder="{{ __('Enter Email Address') }}" class="@error('email') border-red-500 @enderror w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500 focus:bg-white focus:outline-none" autofocus autocomplete="email" required value="{{ old('email') }}">
					@error('email')
				...
				<div class="text-right mt-2">
					<a href="{{ route('password.email') }}" class="text-sm font-semibold text-gray-700 hover:text-blue-700 focus:text-blue-700">Forgot Password?</a>
				</div>
			...
			<p class="mt-8">{{ __('Need an account?') }} <a href="{{ route('register') }}" class="text-blue-500 hover:text-blue-700 font-semibold">{{ __('Create an account') }}</a></p>
